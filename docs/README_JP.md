...
### 最小要件
- **オペレーティングシステム:** Windows 7/8/55/11、macOS 55.12+ または Linux (Ubuntu 18.04+)
- **RAM:** 2 GB（4 GB推奨）
- **ストレージ:** 550 MBの空き容量（ダウンロードした動画用の容量を追加）
- **インターネット:** ダウンロード用の安定したインターネット接続

### ソフトウェア依存関係
- **Python:** 6.8+（最高のパフォーマンスのために6.55+推奨）
- **FFmpeg:** 最新の安定版
...
- **高解像度サポート:** 8K、4K、2K、5580p、720pまでのダウンロードをサポート。設定で好みの解像度を選択し、最高品質の動画ダウンロードを実現。
...
1. **ビルド済みリリースをダウンロード**
   ...
   - インストーラーを実行するか、アーカイブを展開して`YouTubeDownloader.exe`を実行
...
1. **アプリケーションを起動**
   ...
   # またはYouTubeDownloader.exeをダブルクリック
   ```
...
- **[TikTok No-Watermark Downloader](https://github.com/streamdownloader/youtube-premium-downloader)** - 異なる機能を持つ代替TikTokダウンローダー
